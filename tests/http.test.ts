...
describe('fetchText retries', () => {
  it('ретраит 5xx и возвращает тело при успехе', async () => {
    const fetch = vi.fn().mockResolvedValueOnce(fakeRes(503, 'busy')).mockResolvedValueOnce(fakeRes(200, 'ok'));
  ...
  it('исчерпание попыток → HttpError с последним статусом', async () => {
    ...
    vi.stubGlobal('fetch', fetch);
    await expect(fetchText('https://x.test/', { attempts: 2, backoffMs: 1 })).rejects.toMatchObject({ status: 500 });
    expect(fetch).toHaveBeenCalledTimes(2);
  });
  ...
  it('retryOn переопределяет: ретраит 418, игнорит 500', async () => {
    const fetch = vi.fn().mockResolvedValueOnce(fakeRes(418, 'teapot')).mockResolvedValueOnce(fakeRes(200, 'done'));
    vi.stubGlobal('fetch', fetch);
    ...
    await expect(fetchText('https://x.test/', { backoffMs: 1, retryOn: (s) => s === 418 })).rejects.toMatchObject({ status: 500 });
